...
** Accessing the system in non-components
If needed, the running `system` can be accessed by importing the `system` var from `reloaded.repl`:
#+BEGIN_SRC clojure
(ns front-end.webapp.handler
 (:require [reloaded.repl :refer [system]]))
#+END_SRC

When using [[https://github.com/ptaoussanis/sente][Sente]] for example, you must define a number of specific routes (e.g. /chsk) in your ring handler. The problem is that those routes are served by callbacks provided by the [[https://github.com/danielsz/system/blob/master/src/system/components/sente.clj][sente component]]. 

One option would to make the ring handlers themselves a component which depends on the sente component. Another is to directly access the system in your ring handler:
...
As ever, it is a trade off - "golden hammer - everything must be a component" or global vars.

...
** Boot-system
~System~ and ~Boot~ are a match made in heaven, allowing you to craft yourself very comfortable workflows.
The available options can be listed like so: 
#+BEGIN_SRC shell
$ boot system -h
#+END_SRC
...
A [[https://github.com/danielsz/holygrail][tutorial]] is available in a separate repository, demonstrating the advanced workflows made possible by combining ~system~ with ~Boot~. 

#+END_CENTER
You can tailor  some very comfortable workflows when using ~Boot~ together with ~system~.

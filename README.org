...
- Interactive programming via the REPL.
- Restartable system. What warrants a system restart is user-configurable. File-based granularity.
- Manual and automatic mode, ie. either you manipulate the system in the REPL, or you configure it to react to editing changes. Changes that do not affect the system are available immediately via namespace reloading.
- Hot-reloading in the browser

The system task is invoked like any boot task, and its options are queried like so: 
#+BEGIN_SRC shell
$ boot system -h
#+END_SRC
...
A [[https://github.com/danielsz/holygrail][tutorial]] is available in a separate repository, demonstrating some advanced trickery.

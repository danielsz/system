...
It is important to understand that code reloading and system restarts are orthogonal—both are desirable properties of a programming environment. A full restart is a blunt tool. No need to restart the web server just because a pure helper function was modified. 

We *don’t want* to restart the system with *every* code change. Ideally, we want to be able to only reload code that has changed, and only restart when necessary. 

~boot-system~ gives you finegrained tuning over system restarts vs code reload. Each time you change a file, ~boot-system~ will reload your code with ~refresh~ in  ~tools.namespace~. Filenames that appear in the ~files~ vector will trigger a full restart (with ~reset~ in ~component~). Typically, that vector will be small, as most modifications do not warrant a full restart. An example of when you would want a full restart is when you change routes; you will want the web server to be restarted. Another example is when you change a protocol; you would want to recreate all instances of records implementing that protocol.

In summary, when you tell boot-system to manage your application lifecycle (with the ~auto~ option), either one of those two things will happen after you change a source file: 
- ~refresh~ will first unload all affected files in dependency order before reloading them.
- ~reset~ will restart the system if that file was defined in the ~files~ vector.

*** The Holy Grail

With ~system~, you can enjoy a true Lisp environment where everything is reloadable or restartable, helping you to keep your flow when developping applications. A [[https://github.com/danielsz/holygrail][tutorial]] is available in a separate repository.
